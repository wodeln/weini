  ...
  <div
    ...
    <footer
      ...
      <div
        ...
        :class="{ dis: relation.status!=='eachFollow'}">
        <svg class="m-style-svg m-svg-def">
          <use
            xmlns:xlink="http://www.w3.org/1999/xlink"
...
export default {
  ...
  methods: {
     ...
     */
    startSingleChat() {
      if(this.relation.status=='eachFollow'){
          this.$nextTick(() => {
              this.$router.push(`/chats/${this.userID}`);
          });
      }else {
          this.$Message.error("发起聊天需要相互关注哦");
      }
    },
    sendMessage(){
      if(true){
          this.$nextTick(() => {
              this.$router.push(`/private/${this.userID}`);
          });
      }else{
          this.$Message.error("你不能发私信给对方");
      }
    },
    rewardUser() {
      bus.$emit("reward", {
